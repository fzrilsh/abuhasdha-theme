wp.customize.section('gallery_section', function (section) {
    section.expanded.bind(function (isExpanded) {
        if (isExpanded) {
            initGalleryCustomizer();
        }
    });
});

function initGalleryCustomizer() {
    const $ = jQuery;
    const field = $('[data-customize-setting-link="custom_gallery_images"]');

    if (!field.length) {
        console.warn('Gallery field not found (yet)');
        return;
    }
    ...
    preview.on('click', '.remove-image', function () {
        ...
        field.val(newIds.join(',')).trigger('change');
        renderPreview();
    });
}